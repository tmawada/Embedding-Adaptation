"""Query distillation inference demo: the same test as ../inference.py (Query-DANN v2), for the distillation adapter.

Live BGE-M3 query encoding + trained distillation adapter, search over the cached corpus.
...
Sampled mode (default): draws N queries from a split and, for each, reports
  ...
  - nDCG@10 / MRR@10 / Recall@100 for the same three conditions
...
With the same --seed/--split/--n as ../inference.py it samples the same queries, so outputs are directly comparable.

  python3 inference.py --n 5
  python3 inference.py --query "presiden pertama indonesia siapa sih"
"""
...
SEARCH_DEPTH = 100
METHOD = "Query distillation"
CONDITION_LABELS = {"base_informal": "base BGE-M3 (informal)", "adapted_informal": f"{METHOD} (informal)",
                    "base_formal": "base BGE-M3 (formal)"}


class DistillationRetriever:
    """Frozen BGE-M3 query encoder + distillation adapter over pre-encoded corpus embeddings."""
    ...
    def encode(self, texts: List[str]) -> torch.Tensor:
        return torch.from_numpy(self.encoder.encode(texts, max_length=64, progress=False)).to(self.device).float()

    def adapt(self, z: torch.Tensor) -> torch.Tensor:
        return adapt(self.adapter, z)
...
def print_topk(title: str, scores, rows, retriever: DistillationRetriever, k: int, relevant: set = None):
    print(f"  {title}")
    for r in range(k):
...
def sampled_demo(retriever: DistillationRetriever, args):
    ...
    for i, qid in enumerate(qids):
        relevant = sample_qrels[qid]
        gold = torch.stack([retriever.doc_embedding(d) for d in sorted(relevant)])
        rec = {
            "method": "query_distillation", "query_id": qid, "formal_query": formal[i], "informal_query": informal[i],
        ...
        print_topk(f"top-{args.k} {METHOD} (informal):", s_adapt[i], r_adapt[i], retriever, args.k, relevant)
    ...
    print("\n" + "=" * 118)
    print(f"SUMMARY over {n} sampled {args.split} queries ({METHOD})")
...
def free_text_demo(retriever: DistillationRetriever, args):
    z_base = retriever.encode(args.query)
    z_adapt = retriever.adapt(z_base)
    ...
    for i, q in enumerate(args.query):
        ...
        print_topk(f"top-{args.k} {METHOD}:", s_adapt[i], r_adapt[i], retriever, args.k)
        ids = retriever.store.doc_ids
        records.append({"method": "query_distillation", "query": q, "cos_base_vs_adapted": cos(z_base[i], z_adapt[i]),
...
def main():
    ...
    retriever = DistillationRetriever(args.ckpt, args.cache_dir, args.data_dir)
